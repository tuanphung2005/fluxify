...
export default function PaymentSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.put("/api/vendor/payment", {
        ...
        bankAccountName: removeVietnameseDiacritics(
          formData.bankAccountName,
        ).toUpperCase(),
      });
  ...
  const hasValidSettings =
    formData.bankId && formData.bankAccount && formData.bankAccountName;
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div>
          ...
          <p className="text-default-500 mt-1">
            Cấu hình thông tin ngân hàng để nhận thanh toán từ khách hàng qua
            VietQR
          </p>
        </div>
            ...
            <CardBody className="space-y-4">
              <Autocomplete
                ...
                onSelectionChange={(key) =>
                  handleChange("bankId", (key as string) || "")
                }
              >
                {(bank) => (
                  <AutocompleteItem
