...
export async function GET(req: NextRequest) {
  try {
    ...
    orderItems.forEach((item) => {
      ...
      revenueByDate[date] =
        (revenueByDate[date] || 0) + Number(item.price) * item.quantity;
    ...
    const productRevenue: Record<
      string,
      { product: (typeof products)[0]; totalSold: number; totalRevenue: number }
    > = {};
    ...
    orderItems.forEach((item) => {
      ...
      if (productRevenue[item.productId]) {
        ...
        productRevenue[item.productId].totalRevenue +=
