...
export default function ShopOverview({ vendorId }: ShopOverviewProps) {
  ...
  const stats = [
    ...
    },
    {
      ...
      value:
        data.avgProductRating > 0 ? data.avgProductRating.toFixed(1) : "N/A",
    ...
    },
    {
      ...
      value:
        data.avgShippingRating > 0 ? data.avgShippingRating.toFixed(1) : "N/A",
  ...
  return (
    <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-6">
      {stats.map((stat, index) => (
        <Card
          key={index}
          className="border-none shadow-md bg-gradient-to-br from-default-50 to-default-100 hover:shadow-lg transition-shadow"
        >
          <CardBody className="p-4 flex flex-row items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <div className="mt-1 flex items-center gap-2">
                {stat.showStars ? (
                  <StarRating showValue rating={stat.rating!} size="sm" />
                ) : (
                  <p className="font-bold text-xl leading-none">{stat.value}</p>
                )}
              </div>
              {stat.subtext && (
                <p className="text-[10px] text-default-400 mt-1 uppercase tracking-wider font-semibold">
                  {stat.subtext}
                </p>
              )}
            </div>
