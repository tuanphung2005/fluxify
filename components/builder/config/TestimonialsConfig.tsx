...
interface TestimonialsConfigPanelProps {
  ...
  onUpdate: (
    field: string | Partial<TestimonialsConfig>,
    value?: unknown,
  ) => void;
}

export default function TestimonialsConfigPanel({
  config,
  onUpdate,
}: TestimonialsConfigPanelProps) {
        ...
        <div key={index} className="p-3 bg-default-100 rounded-lg space-y-3">
          ...
          <Select
            ...
            onSelectionChange={(keys) =>
              updateTestimonial(index, "rating", Number(Array.from(keys)[0]))
            }
          >
            {[5, 4, 3, 2, 1].map((r) => (
              <SelectItem key={String(r)}>
                {"⭐".repeat(r)} ({r})
              </SelectItem>
            ))}
          </Select>
